...
const WorkoutEditMode = (props) => {
    ...
    const[workoutsToUpdate, setWorkoutsToUpdate] = useState([workoutsToEdit.current]);
    let editOptions =[];
    let editArray = [];
    let testArray = [];
    ...
    useEffect(()=> {
        const fetchWorkouts = async () => {
            ...
            try {
                ...
                const getWorkoutsDay = workouts.map(w => w.day === currentDay - 2);
                ...
                createUpdate(editArray)
                // workouts.map(w => {
                //     if(w.day === currentDay - 2 || currentDay - 1){
                //         testArray.push(w)
                //         console.log(testArray)
                //         createUpdate(testArray)
                //     }
                // })
            } catch (err){}
        }
    ...
    //     loggedMonth={workoutsToUpdate} />
    // )
    
    return(
        <WorkoutOutput
        workoutItems={workoutsToUpdate} />
    )
};
