        ...
        <React.Fragment>
            ...
            <AnimatePresence>
                ...
                <header className="header">
                    <motion.h3
                    whileTap={{ scale: 0.8 }} 
                    onClick={accordionToggle} className="about_me_header" id={methodTitle}>{methodTitle} <MdArrowDropDownCircle /> </motion.h3>
                </header>
                </motion.div>
                <motion.div className="bio_info">
                <header className="header">
                    <motion.h3
                    whileTap={{ scale: 0.8 }} 
                    onClick={accordionToggle} className="about_me_header" id={bioTitle}>{bioTitle} <MdArrowDropDownCircle /> </motion.h3>
                </header>
                </motion.div>
                </motion.div>
                <AnimatePresence>
                {isOpen && !flip && ( <CardFlip items={items} />) }
